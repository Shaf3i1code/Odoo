        ...
        for product in productions.mapped('product_id'):
            ...
            for move in byproduct_moves:
                ...
                if move.cost_share != 0:
                    ...
                    total_cost_by_product[move.product_id] += cost_share
                    component_cost_by_product[move.product_id] += total_cost_by_mo[move.production_id.id]  / cost_share[move.production_id.id]
                    operation_cost_by_product[move.product_id] += 0

            # Get product qty and its relative total + avg per uom cost share amount
            uom = product.uom_id
            mo_qty = 0
            for m in mos:
                cost_share = sum(m.move_finished_ids.mapped('cost_share'))
                total_cost_by_product[product] += (total_cost_by_mo[m.id] - cost_share) if total_cost_by_mo[m.id] > cost_share else (cost_share - total_cost_by_mo[m.id])
                component_cost_by_product[product] += (component_cost_by_mo[m.id] - cost_share) if component_cost_by_mo[m.id] > cost_share else (cost_share - component_cost_by_mo[m.id])
                operation_cost_by_product[product] += operation_cost_by_mo[m.id] - cost_share if operation_cost_by_mo[m.id] > cost_share else (cost_share - operation_cost_by_mo[m.id])
