...
class ReportMoOverview(models.AbstractModel):
    ...
    def _get_cost_breakdown_data(self, production, extras, remaining_cost_share):
        ...
        for bp_move in production.move_byproduct_ids:
            ...
            cost_share = bp_move.cost_share
            total_cost_by_product[bp_move.product_id] += cost_share
            component_cost_by_product[bp_move.product_id] += cost_share
            operation_cost_by_product[bp_move.product_id] += cost_share
    ...
    def _get_unit_cost(self, move):
        ...
        if svl:
            if svl.quantity:
                return svl.value / svl.quantity
            return 0.0
        return move.price_unit or 0.0
    #def _get_unit_cost(self, move):
    #    if not move:
    #        return 0.0
    #    return move.price_unit #or move.product_id.uom_id._compute_price(move.product_id.standard_price, move.product_uom)
    ...
    def _get_byproducts_data(self, production, current_mo_cost, current_bom_cost, current_real_cost, level=0, current_index=False):
        ...
        for index, move_bp in enumerate(production.move_byproduct_ids):
            ...
            cost_share = move_bp.cost_share 
            byproducts_cost_portion += cost_share
            mo_cost = cost_share
            bom_cost = cost_share
            real_cost = cost_share
            total_mo_cost += mo_cost
            total_bom_cost = self._sum_bom_cost(total_bom_cost, bom_cost)
            total_real_cost += real_cost
            if self._is_production_started(production):
    ...
    def _get_replenishment_lines(self, production, move_raw, replenish_data, level, current_index):
        ...
        for count, forecast_line in enumerate(current_lines):
            ...
            if doc_in._name == 'mrp.production':
                ...
                replenishment['summary']['mo_cost'] = initial_mo_cost 
                replenishment['summary']['bom_cost'] = initial_bom_cost 
                replenishment['summary']['real_cost'] = initial_real_cost 
