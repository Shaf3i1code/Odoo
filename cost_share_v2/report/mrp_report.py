    ...
    def _select(self):
        select_str = f"""
            SELECT
                ...
                comp_cost.total - cost_share.byproduct_cost_share / prod_qty.product_qty * account_currency_table.rate                    AS unit_component_cost,
                op_cost.total - cost_share.byproduct_cost_share / prod_qty.product_qty * account_currency_table.rate                      AS unit_operation_cost,
                ({self._select_total_cost()}) - cost_share.byproduct_cost_share / prod_qty.product_qty * account_currency_table.rate      AS unit_cost,
                op_cost.total_duration / prod_qty.product_qty                                                                           AS unit_duration,
                cost_share.byproduct_cost_share * account_currency_table.rate                                   AS byproduct_cost
    ...
    def _join_byproducts_cost_share(self):
        return """
            LEFT JOIN (
                SELECT
                    mo.id AS mo_id,
                    COALESCE(SUM(sm.cost_share), 0.0) AS byproduct_cost_share
